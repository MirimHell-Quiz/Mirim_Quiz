let nameuser = json_encode($uname);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function generateRandomNickname() {
        const nicknames = [];//php에서 랜덤 이름 값 받기
    ...
    function printReview(text) { // 리뷰 출력 함수
        ...
        if (text.trim() !== "") {
            ...
            const messageHTML = `
                <div class="information neon">
                    <div class="nickname neon-text-2 neon">${nickname}</div>
                    <div class="reviewOutput neon-text-2">${text}</div>
                </div>
            `;
